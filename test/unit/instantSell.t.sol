...
contract TestInstantSell is Test, OffersLoansFixtures, ISellerFinancingEvents {
    ...
    function _test_instantSell_loanClosed_simplest_case(FuzzedOfferFields memory fuzzed) private {
        ...
        vm.expectEmit(true, true, false, false);
        emit InstantSell(offer.item.token, offer.item.identifier, 0);
        ...
        assertEq(
            ...
            (buyer1BalanceBefore - offer.terms.downPaymentAmount + minProfitAmount)
        );
    }
    ...
    function _test_instantSell_loanClosed_multipleConsideration(FuzzedOfferFields memory fuzzed) private {
        ...
        assertEq(
            address(buyer1).balance,
            (buyer1BalanceBefore - offer.terms.downPaymentAmount)
        );
    }
    ...
    function _test_instantSell_reverts_ifOrderConsideration0NotERC721Type(
        ...
        sellerFinancing.instantSell(offer.item.token, offer.item.identifier, 0, abi.encode(order[0]));
        vm.stopPrank();
    }
