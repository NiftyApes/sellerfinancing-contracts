...
contract TestMakePayment is Test, OffersLoansFixtures, ISellerFinancingEvents {
    ...
    function _test_makePayment_fullRepayment_simplest_case(
        ...
        emit PaymentMade(
            offer.nftContractAddress,
            offer.nftId,
            loan.remainingPrincipal + periodInterest + protocolInterest,
    ...
    function _test_makePayment_fullRepayment_protocolInterest(
        FuzzedOfferFields memory fuzzed
    ) private {
        Offer memory offer = offerStructFromFields(fuzzed, defaultFixedOfferFields);
    ...
    function test_fuzz_makePayment_fullRepayment_protocolInterest(
        FuzzedOfferFields memory fuzzed
    ) public validateFuzzedOfferFields(fuzzed) {
        _test_makePayment_fullRepayment_protocolInterest(fuzzed);
    }

    function test_unit_makePayment_fullRepayment_protocolInterest() public {
        FuzzedOfferFields memory fixedForSpeed = defaultFixedFuzzedFieldsForFastUnitTesting;
        _test_makePayment_fullRepayment_protocolInterest(fixedForSpeed);
    }
    ...
    function _test_makePayment_partialRepayment_simplest_case(
        ...
        ) = sellerFinancing.calculateMinimumPayment(loan);

        (address payable[] memory recipients, uint256[] memory amounts) = IRoyaltyEngineV1(
            0x0385603ab55642cb4Dd5De3aE9e306809991804f
        ).getRoyalty(offer.nftContractAddress, offer.nftId, totalMinimumPayment - protocolInterest);
        ...
        assertEq(
            sellerBalanceAfter,
            (sellerBalanceBefore + totalMinimumPayment - totalRoyaltiesPaid - protocolInterest)
        );
        ...
        assertEq(
            ...
            loan.remainingPrincipal - (totalMinimumPayment - periodInterest - protocolInterest)
        );
    ...
    function _test_makePayment_partialRepayment_withProtocolFee(
        ...
        vm.assume(offer.downPaymentAmount < ~uint128(0));
    ...
    function test_fuzz_makePayment_partialRepayment_withProtocolFee(
        ...
        _test_makePayment_partialRepayment_withProtocolFee(fuzzed);
    }

    function test_unit_makePayment_partialRepayment_withProtocolFee() public {
        FuzzedOfferFields memory fixedForSpeed = defaultFixedFuzzedFieldsForFastUnitTesting;
        _test_makePayment_partialRepayment_withProtocolFee(fixedForSpeed);
    }
    ...
    function _test_makePayment_fullRepayment_in_gracePeriod(
        ...
        (, uint256 totalInterest, uint256 protocolInterest2) = sellerFinancing
            .calculateMinimumPayment(loan);

        assertEq(totalInterest, 2 * periodInterest);
        assertEq(protocolInterest2, 2 * protocolInterest);

        vm.startPrank(buyer1);
        sellerFinancing.makePayment{
            value: (loan.remainingPrincipal + totalInterest + protocolInterest2)
        }(offer.nftContractAddress, offer.nftId);
        vm.stopPrank();
    ...
    function _test_makePayment_reverts_if_post_grace_period(
        ...
        assertEq(protocolInterest2, 3 * protocolInterest);
    ...
    function _test_makePayment_partialRepayment_in_grace_period(
        ...
        assertEq(
            totalPeriodInterest + totalProtocolInterest,
            2 * (periodInterest + protocolInterest)
        );
        assertEq(
            totalMinimumPayment,
            2 * loan.minimumPrincipalPerPeriod + totalPeriodInterest + totalProtocolInterest
        );
        ...
        (address payable[] memory recipients, uint256[] memory amounts) = IRoyaltyEngineV1(
            0x0385603ab55642cb4Dd5De3aE9e306809991804f
                ...
                offer.nftContractAddress,
                offer.nftId,
                totalMinimumPayment - totalProtocolInterest
            );
        ...
        assertEq(
            sellerBalanceAfter,
            (sellerBalanceBefore + totalMinimumPayment - amounts[0] - totalProtocolInterest)
        );
        ...
        assertEq(
            ...
            loan.remainingPrincipal -
                (totalMinimumPayment - totalPeriodInterest - totalProtocolInterest)
        );
    ...
    function _test_makePayment_reverts_ifCallerSanctioned(FuzzedOfferFields memory fuzzed) private {
        Offer memory offer = offerStructFromFields(fuzzed, defaultFixedOfferFields);
    ...
    function _test_makePayment_reverts_ifAmountReceivedLessThanReqMinPayment(
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                loan.minimumPrincipalPerPeriod + periodInterest + protocolInterest - 1,
                loan.minimumPrincipalPerPeriod + periodInterest + protocolInterest
            )
        );
        sellerFinancing.makePayment{
            value: (loan.minimumPrincipalPerPeriod + periodInterest + protocolInterest - 1)
        }(offer.nftContractAddress, offer.nftId);
        vm.stopPrank();
    }
    ...
    function _test_makePayment_returns_sellerValueIfSnactioned(
        ...
        vm.prank(seller1);
        IERC721Upgradeable(address(sellerFinancing)).transferFrom(
            seller1,
            SANCTIONED_ADDRESS,
            loan.sellerNftId
        );
        ...
        assertEq(
            buyer1BalanceAfterPayment,
            (buyer1BalanceBeforePayment - royaltiesPaidInMakePayment - protocolInterest)
        );
    }
